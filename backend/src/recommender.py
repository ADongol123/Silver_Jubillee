...
def init_group_cache():
    ...
    for group in groups:
        ...
        # Ensure 'description' exists, default to empty string if not
...
def get_user_profile(user_id: str):
    ...
    # Use interests and hobbies if they exist, default to empty strings
    interests = user.get("interests", "") if isinstance(user.get("interests", ""), str) else " ".join(user.get("interests", []))
    hobbies = user.get("hobbies", "") if isinstance(user.get("hobbies", ""), str) else " ".join(user.get("hobbies", []))
    return f"{interests} {hobbies}".lower().replace(",", " ")
...
def recommend_groups(user_id: str, top_n: int = 3):
    ...
    return [{"group_id": group_ids[i], "name": group_names[i], "similarity": float(similarities[i])} for i in top_indices]
...
def get_recommendations(user_id):
    try:
        ...
        # Handle Werkzeug exceptions (like abort) properly
